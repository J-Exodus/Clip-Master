...
def settings_check(tkapp):
    ...
    if not os.path.exists(home + IN_PATH):
        sys.exit(
            "Source path given for media does not exist. Please ensure "
    ...
    if os.path.isfile(home + "/cut_list.txt"):
        ...
        tkapp.info_label.config(
            text="A list for processing already exists.\r"
...
def get_info():
    ...
    try:
        ...
        sys.exit(
            "Can not connect with VLC web interface. Please check "
...
def write_to_file():
    ...
    app.info_label.config(
        text="Clip successfully saved for processing.\r" "Ready to mark next clip..."
    )
...
def mark_clip():
    ...
    if timeIn == "0":
        ...
        if mark_in[0] == 0 and mark_in[1] == 0:
            info_msg = "Clip start position marked at {0} " "seconds.".format(
                mark_in[2]
            )
        elif mark_in[0] == 0:
            info_msg = "Clip start position marked at {0} mins, {1} " "seconds.".format(
                mark_in[1], mark_in[2]
            )
        else:
            info_msg = (
                "Clip start position marked at {0} hours, {1} mins, "
                "{2} seconds.".format(mark_in[0], mark_in[1], mark_in[2])
            )
...
class TkInterface(tkinter.Tk):
    def __init__(self, parent):
        ...
        self.save_button = tkinter.Button(
            self, text=u"Save clip for " "processing", command=write_to_file
        )
        self.rest_button = tkinter.Button(self, text="Reset", command=reset_markers)
        self.process_clips = tkinter.Button(
            self, text="Finish and process " "marked clips", command=process_clips
        )
        self.info_label = tkinter.Label(
            self, text="Ready to mark clips...", fg="white", bg="steel blue", pady=10
        )
